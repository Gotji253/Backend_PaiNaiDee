...
from app.main import app as main_app # Main FastAPI application
from app.db.database import Base # SQLAlchemy Base for table creation/deletion
from app.core.config import settings # Application settings
from app.db.database import get_db # Original get_db dependency

# Use the test database URL from settings
SQLALCHEMY_TEST_DATABASE_URL = settings.TEST_DATABASE_URL

engine = create_engine(SQLALCHEMY_TEST_DATABASE_URL, connect_args={"check_same_thread": False} if SQLALCHEMY_TEST_DATABASE_URL.startswith("sqlite") else {})
...
def create_test_database():
    ...
    if SQLALCHEMY_TEST_DATABASE_URL.startswith("sqlite:///") and os.path.exists(SQLALCHEMY_TEST_DATABASE_URL.split(":///")[1]):
...
@pytest.fixture(scope="function") # Changed from module to function for cleaner test isolation
...
@pytest.fixture(scope="module") # Or function, depending on how often you need new tokens
async def test_user_token(client: AsyncClient, db: Session) -> str:
    ...
    if not user:
        user_in_create = UserCreate(username=test_username, email=test_username, password=test_password)
    ...
    if response.status_code != 200:
        ...
        raise Exception(f"Could not log in test user {test_username} to get token. Status: {response.status_code}, Response: {response.text}")

...
class CRUDItinerary:
    ...
    def get_itineraries_by_user(self, db: Session, user_id: int, skip: int = 0, limit: int = 20) -> List[Itinerary]:
        return db.query(Itinerary).filter(Itinerary.user_id == user_id).offset(skip).limit(limit).all()

    def create_itinerary(self, db: Session, *, itinerary_in: ItineraryCreate, user_id: int) -> Itinerary:
    ...
    def update_itinerary(self, db: Session, *, db_itinerary: Itinerary, itinerary_in: ItineraryUpdate) -> Itinerary:
        ...
        if "place_ids" in update_data:
            ...
            if place_ids is not None: # Check if it's None, meaning no change, or empty list to clear
                # Fetch Place objects for the new list of IDs
                places = db.query(Place).filter(Place.id.in_(place_ids)).all()
                db_itinerary.places_in_itinerary = places # Replace existing places with the new list
    ...
    def add_place_to_itinerary(self, db: Session, itinerary_id: int, place_id: int) -> Optional[Itinerary]:
    ...
    def remove_place_from_itinerary(self, db: Session, itinerary_id: int, place_id: int) -> Optional[Itinerary]:
        itinerary = self.get_itinerary(db, itinerary_id)
        place = db.query(Place).get(place_id)
        if itinerary and place:

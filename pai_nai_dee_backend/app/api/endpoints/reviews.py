...
def create_review(
    *,
    ...
    review = crud.crud_review.create_review(db=db, review_in=review_in, user_id=current_user.id)
...
def read_reviews_for_place(
    ...
    if not place:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Place not found")
    reviews = crud.crud_review.get_reviews_by_place(db, place_id=place_id, skip=skip, limit=limit)
    return reviews
...
def read_reviews_by_user(
    ...
    skip: int = 0,
    limit: int = 20,
    current_user: UserModel = Depends(get_current_active_user) # Auth: only user themselves or admin
) -> Any:
    """
    ...
    reviews = crud.crud_review.get_reviews_by_user(db, user_id=user_id, skip=skip, limit=limit)
    return reviews
...
def update_review(
    *,
    ...
    if db_review.user_id != current_user.id: # and not crud.user.is_superuser(current_user): # Add admin override later
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not enough permissions to update this review")

    review = crud.crud_review.update_review(db=db, db_review=db_review, review_in=review_in)
    # crud.crud_place.update_place_average_rating(db, place_id=review.place_id) # Example
    return review
...
def delete_review(
    *,
    ...
    if db_review.user_id != current_user.id:
        ...
        print(f"User {current_user.id} attempting to delete review {db_review.id} owned by {db_review.user_id}")
    ...
    if not deleted_review: # Should not happen if previous check passed
        raise HTTPException(status_code=404, detail="Review not found during delete operation")

...
def create_itinerary(
    *,
    ...
    itinerary = crud.crud_itinerary.create_itinerary(db=db, itinerary_in=itinerary_in, user_id=current_user.id)
    return itinerary
...
def read_itinerary_by_id(
    itinerary_id: int,
    db: Session = Depends(get_db),
    current_user: UserModel = Depends(get_current_active_user) # Requires auth for permission check
) -> Any:
    """
    ...
    if itinerary.user_id != current_user.id: # and not crud.user.is_superuser(current_user):
...
def update_itinerary(
    *,
    ...
    if db_itinerary.user_id != current_user.id: # and not crud.user.is_superuser(current_user):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not enough permissions")
...
def delete_itinerary(
    *,
    ...
    deleted_itinerary = crud.crud_itinerary.delete_itinerary(db=db, itinerary_id=itinerary_id)
    if not deleted_itinerary: # Should not happen
        raise HTTPException(status_code=404, detail="Itinerary not found during delete operation")
...
def add_place_to_itinerary_endpoint(
    ...
    updated_itinerary = crud.crud_itinerary.add_place_to_itinerary(db, itinerary_id, place_id)
...
def remove_place_from_itinerary_endpoint(
    ...
    place = crud.crud_place.get_place(db, place_id=place_id)
    if not place: # Technically, crud.remove_place_from_itinerary handles this, but good to check early.
        raise HTTPException(status_code=404, detail="Place not found to remove")

    updated_itinerary = crud.crud_itinerary.remove_place_from_itinerary(db, itinerary_id, place_id)
    if not updated_itinerary : # This might happen if place was not in itinerary.
        ...
        # For consistency, let's ensure it's not None (which means itinerary itself wasn't found by CRUD)
         raise HTTPException(status_code=500, detail="Could not remove place from itinerary")
    return updated_itinerary

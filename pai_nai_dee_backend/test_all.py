...
def database_setup_logic():
    ...
    if IS_CI:
        print(f"CI Environment: Using DATABASE_URL from settings: {settings.DATABASE_URL}")
        ...
        if PostgresContainer is None:
            print(f"Local Environment: testcontainers not found. Using TEST_DATABASE_URL: {settings.TEST_DATABASE_URL}")
            SQLALCHEMY_DATABASE_URL_FOR_TESTS = settings.TEST_DATABASE_URL
            connect_args = {"check_same_thread": False} if "sqlite" in SQLALCHEMY_DATABASE_URL_FOR_TESTS else {}
            _test_engine = create_engine(SQLALCHEMY_DATABASE_URL_FOR_TESTS, connect_args=connect_args)
            _TestingSessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=_test_engine)
            ...
            try:
                with PostgresContainer("postgres:14-alpine") as pg:
                    ...
                    print(f"PostgreSQL container started: {SQLALCHEMY_DATABASE_URL_FOR_TESTS}")
                    _test_engine = create_engine(SQLALCHEMY_DATABASE_URL_FOR_TESTS)
                    _TestingSessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=_test_engine)
                    Base.metadata.drop_all(bind=_test_engine)
                    Base.metadata.create_all(bind=_test_engine)
                ...
                print(f"Falling back to TEST_DATABASE_URL: {settings.TEST_DATABASE_URL}")
                SQLALCHEMY_DATABASE_URL_FOR_TESTS = settings.TEST_DATABASE_URL
                connect_args = {"check_same_thread": False} if "sqlite" in SQLALCHEMY_DATABASE_URL_FOR_TESTS else {}
                _test_engine = create_engine(SQLALCHEMY_DATABASE_URL_FOR_TESTS, connect_args=connect_args)
                _TestingSessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=_test_engine)
...
def test_auth_token(test_user_data): # Depends on test_user_data
    ...
    access_token = create_access_token(
        data={"sub": test_user_data["db_user"].username}, expires_delta=access_token_expires
    )
    return access_token
...
async def test_create_user(client: AsyncClient):
    response = await client.post(f"{settings.API_V1_STR}/users/", json={
        "username": "newuser",
...
@pytest.mark.asyncio
async def test_create_user_duplicate_username(client: AsyncClient, test_user_data): # Uses test_user_data
    response = await client.post(f"{settings.API_V1_STR}/users/", json={
...
@pytest.mark.asyncio
async def test_login_for_access_token(client: AsyncClient, test_user_data): # Uses test_user_data
    login_data = {
...
@pytest.mark.asyncio
async def test_read_users_authenticated(client: AsyncClient, test_auth_token): # Uses test_auth_token
...
@pytest.mark.asyncio
async def test_create_place(client: AsyncClient, test_auth_token): # Uses test_auth_token
    headers = {"Authorization": f"Bearer {test_auth_token}"}
    place_data = {
        "name": "Test Cafe",
    ...
    response = await client.post(f"{settings.API_V1_STR}/places/", json=place_data, headers=headers)
    assert response.status_code == status.HTTP_201_CREATED
    data = response.json()
...
@pytest.mark.asyncio
async def test_create_review_for_place(client: AsyncClient, test_auth_token, db_session): # Uses test_auth_token and db_session
    headers = {"Authorization": f"Bearer {test_auth_token}"}
    place_data = {"name": "Reviewable Place", "category": "Test", "address": "Review Addr"}
    place_response = await client.post(f"{settings.API_V1_STR}/places/", json=place_data, headers=headers)
    ...
    review_response = await client.post(f"{settings.API_V1_STR}/reviews/", json=review_data, headers=headers)
...
@pytest.mark.asyncio
async def test_create_itinerary(client: AsyncClient, test_auth_token, db_session): # Uses test_auth_token and db_session
    headers = {"Authorization": f"Bearer {test_auth_token}"}
    place_data = {"name": "Itinerary Place", "category": "Itinerary Test", "address": "Itin Addr"}
    place_response = await client.post(f"{settings.API_V1_STR}/places/", json=place_data, headers=headers)
    assert place_response.status_code == status.HTTP_201_CREATED
    created_place = place_response.json()
    ...
    itinerary_response = await client.post(f"{settings.API_V1_STR}/itineraries/", json=itinerary_data, headers=headers)
...
@pytest.mark.asyncio
async def test_read_my_itineraries(client: AsyncClient, test_auth_token): # Uses test_auth_token
    headers = {"Authorization": f"Bearer {test_auth_token}"}
    response = await client.get(f"{settings.API_V1_STR}/itineraries/my-itineraries", headers=headers)
    assert response.status_code == status.HTTP_200_OK
    assert isinstance(response.json(), list)
...
@pytest.mark.asyncio
async def test_system_pipeline(client: AsyncClient, test_auth_token, db_session): # Uses all main fixtures
    ...
    # Create a place
    place_data = {"name": "Pipeline Place", "category": "Pipeline", "address":"Pipe Addr"}
    p_res = await client.post(f"{settings.API_V1_STR}/places/", json=place_data, headers=headers)
    assert p_res.status_code == status.HTTP_201_CREATED, f"Failed to create place: {p_res.text}"
    ...
    i_res = await client.post(f"{settings.API_V1_STR}/itineraries/", json=itinerary_data, headers=headers)
    assert i_res.status_code == status.HTTP_201_CREATED, f"Failed to create itinerary: {i_res.text}"
    print("Step 4b: Created itinerary.")

    # Read itineraries to confirm
    my_i_res = await client.get(f"{settings.API_V1_STR}/itineraries/my-itineraries", headers=headers)

...
def create_trip(
    *,
    ...
    trip_create_internal = schemas.TripCreate(**trip_in.model_dump(exclude={"owner_id"}))

    trip = crud.trip.create_with_owner(db=db, obj_in=trip_create_internal, owner_id=current_user.id)
    return trip
...
def read_trip(
    *,
    ...
    if trip.owner_id != current_user.id and not crud.user.is_superuser(current_user): # Allow superuser to access any trip
    ...
    *,
    ...
    if trip_in.owner_id is not None and trip_in.owner_id != trip.owner_id:
        if not crud.user.is_superuser(current_user):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only superusers can change trip ownership.")

    trip = crud.trip.update(db=db, db_obj=trip, obj_in=trip_in)
    return trip

@router.delete("/{trip_id}", response_model=schemas.Trip) # Consider returning 204 No Content
def delete_trip(
    *,
    db: Session = Depends(get_db),
    trip_id: int,
    ...
    if not trip:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Trip not found")
    if trip.owner_id != current_user.id and not crud.user.is_superuser(current_user):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not enough permissions to delete this trip")
...
def add_place_to_trip_route(
    *,
    ...
    if not trip:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Trip not found")
    if trip.owner_id != current_user.id and not crud.user.is_superuser(current_user):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not enough permissions for this trip")
